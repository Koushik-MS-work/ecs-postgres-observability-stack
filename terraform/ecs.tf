############################################
# ECS Cluster (Fargate — no EC2 instances to patch/manage)
############################################
...
resource "aws_ecs_cluster" "main" {
  ...
  setting {
    ...
    value = "enabled" # infrastructure-level CPU/memory/network metrics per task
  }
}
...
resource "aws_iam_role_policy" "ecs_task_logs_metrics" {
  name = "${var.project_name}-${var.environment}-ecs-task-logs-metrics"
  ...
  container_definitions = jsonencode([
    {
      ...
      # Credentials are injected from Secrets Manager at container start —
      # never baked into the image or task definition in plaintext.
      secrets = [
...
############################################
# Autoscaling (application metrics -> scale on CPU/memory/request count)
############################################
...
resource "aws_ecr_repository" "app" {
  ...
  image_scanning_configuration {
    scan_on_push = true # vulnerability scanning for pushed images
  }
